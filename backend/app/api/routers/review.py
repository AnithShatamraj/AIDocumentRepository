...
from app.models.content import Classification, Extraction
...
async def list_queue(
    ...
    if category_id:
        stmt = stmt.where(and_(document_category.c.document_id == ReviewItem.document_id,
                               document_category.c.category_id == category_id))
...
async def resolve(review_id: str, body: ReviewResolve, user: User = Depends(require_reviewer), db: AsyncSession = Depends(get_db)):
    ...
    if item.kind == REVIEW_EXTRACTION and item.extraction_id:
        ext = await db.get(Extraction, item.extraction_id)
        if not ext:
        ...
        if body.action in ("accept", "modify") and body.category_ids:
            await db.execute(document_category.delete().where(document_category.c.document_id == item.document_id))
            for cid in body.category_ids:
                await db.execute(document_category.insert().values(document_id=item.document_id, category_id=cid))
                db.add(Classification(tenant_id=user.tenant_id, document_id=item.document_id, category_id=cid,
            ...
            await db.commit()
            # Extraction was skipped for the uncategorized doc — run it now.
            from app.worker.tasks import start_processing
            start_processing.delay(str(item.document_id), STAGE_METADATA)
        elif body.action == "reject":
            pass
        else:
            raise HTTPException(400, "Provide category_ids to accept a classification")
    else:
